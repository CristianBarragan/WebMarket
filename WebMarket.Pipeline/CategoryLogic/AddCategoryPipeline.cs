...
using WebMarket.Pipeline.CategoryLogic.Process.Add;
using WebMarket.Pipeline.CategoryLogic.Process;

namespace WebMarket.Pipeline.CategoryLogic
{
    [ExcludeFromCodeCoverage]
    public class AddCategoryPipeline : LogicPipeline<CategoryParameters>, IAddCategoryPipeline
    {
        private readonly ILogger<CategoryParameters> logger;

        public AddCategoryPipeline(ILogger<CategoryParameters> logger, IMapper mapper, MarketContext marketContext) : base(logger)
        {
            this.logger = logger;
        ...
        public async Task<ApiResponse> ExecutePipeline(CategoryDto model)
        {
            ...
            catch (Exception ex)
            {
                ...
                return new ApiResponse(HttpStatusCode.InternalServerError, $"There was a problem while processing pipeline {typeof(AddCategoryPipeline)}", "");
            }
